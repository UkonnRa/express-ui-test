...
import { AccessListCreateOptions, Journal, JournalRepository } from '@white-rabbit/business-logic/src/domains/journal';
import AuthUser from '@white-rabbit/business-logic/src/shared/auth-user';
import { Role, User, UserRepository } from '@white-rabbit/business-logic/src/domains/user';
import { initMemoryRepositories } from '@white-rabbit/repository-memory';

describe('Accounts', () => {
  initMemoryRepositories();
  const accountRepository: AccountRepository = container.resolve('AccountRepository');
  const journalRepository: JournalRepository = container.resolve('JournalRepository');
  const userRepository: UserRepository = container.resolve('UserRepository');
  const accountService = container.resolve(AccountService);

  const admin = new User({ name: 'User Admin', role: Role.ADMIN, authIds: new Map([['Provider', 'oauthId']]) });
  ...
  Promise.all([
    ...
    ...accounts.map((a) => accountRepository.save(a)),
  ]);
  ...
  it('can query', async () => {
    {
      const result = await accountService.findAllValues(authUser, [{ field: 'name', order: 'ASC' }], {
        size: 10,
        startFrom: 'FIRST',
      });
      expect(result.pageItems.map(({ data }) => data)).toStrictEqual(accounts.map((v) => v.toValue()));
    }
    {
      const result = await accountService.findAllValues(
        ...
        { type: 'AccountQueryFullText', keyword: { fields: ['name'], value: 'Type 2' } },
      );

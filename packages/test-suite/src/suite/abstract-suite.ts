...
import { AccountType, Strategy } from "@white-rabbit/type-bridge";
...
> {
  ...
  protected abstract tasks: Array<Task<T, C, Q>>;
  ...
  start(): void {
    ...
    if (this.tasks.length > 0) {
      each(this.tasks).test(
        `Task ${String(this.type.description)}: $name`,
        async (task: Task<T, C, Q>) => {
          task.setup != null && (await task.setup());
          const authUser = this.queryUsers(task.authUser)[0];
          if (authUser == null) {
            throw new Error(
              `AuthUser with Query[${JSON.stringify(
                task.authUser
              )}] cannot be found`
            );
          }
